...
class WP_Travel_Engine_Emails {
    ...
    public function send_booking_emails( $order_details, $booking_id ) {
        ...
        foreach( $cart_items as $key => $cart_item ) :
            ...
            if ( isset( $cart_item['multi_pricing_used'] ) && $cart_item['multi_pricing_used'] ) :
                ...
                foreach( $cart_item['pax'] as $pax_key => $pax ) :
                    ...
                    $multiple_pricing_html .= '<p>'. $pax_label . ': '. $pax .'X '. wp_travel_engine_get_formated_price_with_currency( $cart_item['pax_cost'][ $pax_key ], null, true ).'</p>';
                endforeach;
        ...
        $default_mail_tags = array(
            ...
            '{tprice}'          => wp_travel_engine_get_formated_price_with_currency( wp_travel_engine_get_actual_trip_price( $trip_id ) ),
            '{price}'           => wp_travel_engine_get_formated_price_with_currency( $order_details['place_order']['cost'] ),
            '{total_cost}'      => wp_travel_engine_get_formated_price_with_currency( $total ),
            '{due}'             => wp_travel_engine_get_formated_price_with_currency( $order_details['place_order']['due'] ),
            '{sitename}'        => $sitename,
