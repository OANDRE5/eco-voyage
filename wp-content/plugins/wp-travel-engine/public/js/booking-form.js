 ...
 */
    ...
    function populateHTML() {
        ...
        $.each(numberFields, function(index, numberField){
            ...
            if ( count > 1 && ( 'group' == type || 'per-group' === pricing_type ) ) {
                count = 1;
            }
            ...
            var priceHtml = wteGetFormatedPriceWithCurrencyCodeSymbol( price, wte.currency.code, wte.currency.symbol );
    ...
    function ApplyCutOffDays( date ) {
        ...
        if ( wte.booking_cutoff.enable && parseInt( wte.booking_cutoff.cutoff ) > 0 ) {
            var today = new Date();

            var today_time = today.getTime() + ( parseInt( wte.booking_cutoff.cutoff ) * 24 * 60 * 60 * 1000 ),
    ...
    function checkAvailableDates(date) {
        ...
        for(var index = 0; index < fixDatesCount; index++) {
            if ( wte_fix_date.seats_available[index][dmy] == '0' || '' == wte_fix_date.seats_available[index][dmy] ) {
                return [false, "", "Unavailable"];
            }
        }
    ...
    $( ".wpte-bf-datepicker" ).datepicker({
        minDate: 0,
        beforeShowDay: (0 == availableDates.length || '' == window.wte_fix_date.enabled) ? ApplyCutOffDays : checkAvailableDates,
        dateFormat: 'yy-mm-dd',
...
function wteGetFormatedPriceWithCurrencyCodeSymbol( price, code, symbol, format, numberOfDecimals ) {
    ...
    var formattedPrice = currency_symbol_display + ' ' + wteGetFormatedPrice( price, format, numberOfDecimals);
