		...
		if ( $screen->post_type == 'trip' || $screen->post_type == 'enquiry' || $screen->post_type == 'booking' || $screen->post_type == 'customer' || $screen->post_type == 'wte-coupon' || isset($_GET['page']) && $_GET['page'] == 'class-wp-travel-engine-admin.php' || $screen->id == 'trip_page_class-wp-travel-engine-admin' || $screen->post_type == 'downloadfile') {
			...
			$extra_array = array(
				'lang' => array(
					...
					'are_you_sure_remove_vid' => __( 'Sure to remove the video from trip gallery?', 'wp-travel-engine' ),
				),
			);
			...
			wp_enqueue_script( $this->plugin_name . '_ui', plugin_dir_url(__FILE__) . 'js'. $asset_script_path. 'wte-admin-ui'. $version_prefix .'.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-datepicker', 'jquery-ui-tabs', 'jquery-ui-sortable', 'toastr.min.js', 'parsley' ), $this->version, true );
