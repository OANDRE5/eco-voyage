...
function wpte_core_ui_fxn($) {
    ...
    $(document).on('click', '.wpte-onoff-block:not(.wpte-floated) .wpte-onoff-toggle', function() {
        $(this).toggleClass('active');
        $(this).siblings('.wpte-onoff-popup').stop(true, false, true).slideToggle();
    });

    //toggle inlined onoff block popup
    $(document).on('click', '.wpte-onoff-block.wpte-floated .wpte-onoff-toggle', function() {
        $(this).toggleClass('active');
        $(this).siblings('.wpte-onoff-popup').fadeToggle();
    });

    //toggle disable class in repeater block
    $(document).on('click', '.wpte-settings .wpte-repeater-block .wpte-system-btns .wpte-toggle-btn', function() {
        $(this).parents('.wpte-repeater-block').toggleClass('wpte-disabled');
    });
    ...
    $( document ).on( 'click', '.wpte_save_continue_link', function(e) {
        ...
        $(parent + ' input, ' + parent + ' select, ' + parent + ' textarea').each(function (index) {
            ...
            if( ! $(this).parsley().isValid() ) {
                invalid = true;
                var Trigger = $(this).parents( '.wpte-tab-content' ).data( 'trigger' );
        ...
        $(parent + ' input, ' + parent + ' select, ' + parent + ' textarea').each(function (index) {
            ...
            filterby_val = $(this).val();
                ...
                }else if ($(this).is("textarea") && $(this).closest(".tmce-active").size() > 0) {
                    id = $(this).attr('id');
    ...
    $(document).on( 'click', '.wpte-add-glb-tab', function(e) {
        ...
        var index  = ( 0 < $( '.wpte-glb-tab-row' ).length ) ? $( '.wpte-glb-tab-row' ).length + 1 : 1 ;
...
function wpte_tab_wpte_overview() {
    jQuery( '.wte-add-trip-highlight' ).on( 'click', function(e) {
        ...
        jQuery( 'input[name="wp_travel_engine_setting[trip_highlights]['+ index +'][highlight_text]"]' ).focus();
        ++index;
...
function wpte_tab_wpte_itinerary() {
    jQuery( '.wpte-add-itinerary' ).on( 'click', function(e) {
        ...
        // Add itinerary.
        var iti_index  = 0 < jQuery( '.wpte-itinerary-repeter' ).length ? jQuery( '.wpte-itinerary-repeter' ).length + 1 : 1;
        var template = wp.template( 'wpte-add-iti-row' );

        jQuery( '.wpte-remove-iti' ).remove();

        jQuery( '#wpte-itinerary-repeter-holder' ).append( template( { key: iti_index } ) );
        ++iti_index;
    ...
    jQuery( document ).on( 'click', '.wpte-remove-iti:last', function(e) {
        ...
        jQuery( '.wpte-itinerary-repeter:last' ).append( '<button class="wpte-delete wpte-remove-iti"></button>' );
    } );
}
/**
 * Trip facts JS.
...
function wpte_tab_wpte_trip_facts() {
    jQuery('body').on('click', '.add-info', function(e) {
        ...
        if (val == '') {
            jQuery('#wte_global_trip_facts').css('-webkit-box-shadow', 'inset 0px 0px 1px 1px red');
            jQuery('#wte_global_trip_facts').css('-moz-box-shadow', 'inset 0px 0px 1px 1px red');
            jQuery('#wte_global_trip_facts').css('box-shadow', 'inset 0px 0px 1px 1px red');
            return;
        } else {
            jQuery('#wte_global_trip_facts').css('-webkit-box-shadow', 'inset 0px 0px 0px 0px red');
            jQuery('#wte_global_trip_facts').css('-moz-box-shadow', 'inset 0px 0px 0px 0px red');
            jQuery('#wte_global_trip_facts').css('box-shadow', 'inset 0px 0px 0px 0px red');
        }
...
function wpte_tab_wpte_gallery() {
    ...
    var allowed_filetype = [
        ...
        file_frame.on('select', function () {
            ...
            selection.map(function (attachment, i) {
                ...
                // Prepare HTML.
                var file_html_content  = '<div class="wpte-gal-img"><input type="hidden" readonly name="wpte_gallery_id['+ index +']" value="' + attachment.id + '"><img src="' + attachment.sizes.thumbnail.url + '" alt=""><div class="wpte-gal-btns"><button class="wpte-change wpte-change-gal-img"></button><button class="wpte-delete wpte-delete-gal-img"></button></div></div>';
    ...
    $(document).on('click', '.wpte-change-gal-img', function (e) {
        ...
        file_frame.on('select', function () {
            ...
            that.parents('.wpte-gal-img').children('input[type="hidden"]').val( attachment.id );
            that.parents('.wpte-gal-img').children('img').attr( 'src', attachment.sizes.thumbnail.url );
        });
        file_frame.open();
    });
    ...
    $(document).on('click', '#wpte-upload-map-img', function (e) {
        ...
        file_frame.on('select', function () {
            ...
            selection.map(function (attachment, i) {
                ...
                $( '#map-image-prev-hldr' ).attr( 'src', attachment.sizes.medium.url );
                $( 'input[name="wp_travel_engine_setting[map][image_url]"]' ).val(attachment.id);
...
function wpte_tab_wpte_faq() {
    //faq toggle
    jQuery(document).on('click', '.wpte-faq-block .wpte-faq-title', function() {
        jQuery(this).parents('.wpte-faq-block').toggleClass('wpte-active');
        jQuery(this).parents('.wpte-faq-title-wrap').siblings('.wpte-faq-content').stop(true, false, true).slideToggle();
    });

    jQuery(document).on( 'click', '.wpte-add-faq-blck', function(e) {
        e.preventDefault();

        var index  = ( 0 < jQuery( '.wpte-faq-block-row' ).length ) ? jQuery( '.wpte-faq-block-row' ).length + 1 : 1 ;
...
(function ($) {
    ...
    function show_admin_tab_content (selector) {
        ...
        selector.parents('.wpte-tab-wrap').siblings('.wpte-tab-content-wrap').children('.wpte-tab-content').removeClass('current');

        selector.parents('.wpte-tab-wrap').siblings('.wpte-tab-content-wrap').children('.' + wpteUniqueClass + '-content').addClass('current content_loaded');
    ...
    jQuery(document).ready(function($) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if ( data.success ) {
                        ...
                        show_admin_tab_content( selector );
                        wpte_do_action( 'wpte_after_admin_tab_shown', content_key );
                        switch( content_key ) {
                            case 'wpte-pricing' :
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if ( data.success ) {
                        ...
                        show_admin_tab_content( selector );
                        wpte_do_action( 'wpte_after_global_settings_tab_shown', content_key );
                        switch( content_key ) {
        ...
        $( document ).on( 'change', 'input[name="wp_travel_engine_setting[multiple_pricing][adult][enable_sale]"]', function(e) {
         ...
         */
        $(document).on('click', '.wpte-add-icon', function() {
        ...
        $(document).on('click', '.wpte-font-awesome-list li', function(event) {
            ...
            $(this).parent().parent().siblings('.trip-tabs-icon').siblings('span.wpte-icon-preview').html( svg_data );
            $(this).parent().parent().fadeOut('slow', function() {
            });
            $(this).parent().parent().siblings('.trip-tabs-icon').siblings('.wpte-font-awesome-list').remove();
            $(this).parents('.wpte-icons-holder').find('.wpte-ico-search').remove();
        });

        $(document).on('keyup', '.wpte-ico-search', function() {
            var value = $(this).val();
            var matcher = new RegExp(value, 'gi');
            $(this).parent('.wpte-font-awesome-list').find('li').show().not(function() {
        ...
        $(document).on( 'click', '.wpte-edit-bkng', function(e) {
            e.preventDefault();
            $(this).parents( '.wpte-block' ).find('.wpte-block-content').fadeOut('slow').css({'height': 0, 'padding-top': 0, 'padding-bottom': 0, 'overflow': 'hidden'});
            $(this).parents( '.wpte-block' ).find('.wpte-block-content-edit').fadeIn('slow').css('height', 'auto');
        } );

        $(document).on( 'click', '.wpte-edit-prsnl-details', function(e) {
            e.preventDefault();
            $(this).parents( '.wpte-prsnl-dtl-blk' ).find('.wpte-prsnl-dtl-blk-content').fadeOut('slow').css({'height': 0, 'margin-top': 0, 'overflow': 'hidden'});
            $(this).parents( '.wpte-prsnl-dtl-blk' ).find('.wpte-prsnl-dtl-blk-content-edit').fadeIn('slow').css('height', 'auto');
        } );
        ...
        $('body').on('change', '.wpte-trp-inf-fieldtyp', function(e) {
            if ($(this).find('select option:selected').val() == 'select') {
                $(this).siblings('.wpte-field').find('.select-options').fadeIn('slow');
            } else {
            ...
            if ($(this).find('select option:selected').val() == 'text' || $(this).find('select option:selected').val() == 'number' || $(this).find('select option:selected').val() == 'textarea') {
                $(this).siblings('.wpte-field').find('.input-placeholder').fadeIn('slow');
            } else {
                $(this).siblings('.wpte-field').find('.input-placeholder').hide();
            }
        });
    ...
    $(function () {
        jQuery(document).ready(function ($) {
            $(document).on('click', '.wpte-rich-textarea', function (e) {
                ...
                var this_id = $(this).find('textarea.wte-editor-area').attr('id');
            ...
            $(document).on("click", ".wp-travel-engine-featured-trip", function (e) {
                e.preventDefault();
                var featuredIcon = $(this);
                var post_id = $(this).attr("data-post-id");
                var nonce = $(this).attr("data-nonce");
                var data = { action: "wp_travel_engine_featured_trip", post_id: post_id, nonce: nonce };
                $.ajax({
                    url: ajaxurl,
                    data: data,
                    type: "post",
            ...
            $(document).on("click", ".wp-travel-engine-featured-term", function (e) {
                e.preventDefault();
                var featuredIcon = $(this);
                var post_id = $(this).attr("data-term-id");
                var nonce = $(this).attr("data-nonce");
                var data = { action: "wp_travel_engine_featured_term", post_id: post_id, nonce: nonce };
                $.ajax({
                    url: ajaxurl,
                    data: data,
                    type: "post",
            ...
            $(document).on('click', '.wpte-tab-wrap .wpte-tab', function() {
                ...
                $(this).parents('.wpte-tab-wrap').siblings('.wpte-tab-content-wrap').children('.wpte-tab-content').removeClass('current');
                $(this).parents('.wpte-tab-wrap').siblings('.wpte-tab-content-wrap').children('.' + wpteUniqueClass + '-content').addClass('current content_loaded');
            });
        });
